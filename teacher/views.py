...
def teacher_add_exam_view(request):
    if request.method=='POST':
        ...
        if sector1:
            ...
            print(skill_name,sector,domain,level,parameters,"?????????????????&&&&&&??????????????????")
            SD=Skills_criteria(skill_name=skill_name,sector=sector,domain=domain,level=level,parameters=parameters)
            SD.save()
    ...
    skills=Skills_criteria.objects.all()
    return render(request,'teacher/teacher_add_exam.html',{'skills':skills})
