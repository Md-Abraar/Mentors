function onUpdateDefault(intersected, nonIntersected, allElements) {
  ...
  export function dragSelect(
    contElem,
    {
      ...
      itemSelector = ".drag-selectable",
      areaClassName = "drag-select-area",
    } = {}
  ) {
    ...
    function onPointerdown(event) {
      ...
      const x1 =
        event.clientX + contElem.scrollLeft - contRect.x - contElem.clientLeft;
      const y1 =
        event.clientY + contElem.scrollTop - contRect.y - contElem.clientTop;
  
      const contElemStyles = getComputedStyle(contElem);
      const rtlLeftOffset = contElem.scrollLeft < 0 ? contElem.scrollLeft : 0;
  
      const onScrollOrBorder =
        x1 <= 0 + rtlLeftOffset ||
        y1 <= 0 ||
        x1 >= contElem.clientWidth + contElem.scrollLeft ||
        y1 >= contElem.clientHeight + contElem.scrollTop;
      const unsupportedTouch =
        event.pointerType === "touch" &&
        contElemStyles["touch-action"] !== "none";
      if (
        isClickInside(event, itemSelector) ||
        unsupportedTouch ||
        onScrollOrBorder
      ) {
        return;
      ...
      function resizeAreaPerFrame(event) {
        if (event.type === "scroll" && !lastPointerEvent) {
          return;
        }
        event.type !== "scroll"
      ...
      function resizeArea(event) {
        ...
        const [intersected, nonIntersected] = checkIntersections(
          areaElem,
          contElem,
          selectableElems
        );
      ...
      addEventListener("scroll", resizeAreaPerFrame, {
        capture: true,
        passive: true,
      });
      contElem.addEventListener("pointermove", resizeAreaPerFrame);
      contElem.addEventListener(
        "lostpointercapture",
        () => {
    ...
    for (const itemElem of selectableElems) {
      ...
      if (
        isRectanglesIntersected(
          {
            x: areaRect.x + contElem.scrollLeft,
            y: areaRect.y + contElem.scrollTop,
            width: areaRect.width,
            height: areaRect.height,
          },
          {
            x: itemRect.x + contElem.scrollLeft,
            y: itemRect.y + contElem.scrollTop,
  ...
  function enableTouchSupport(contElem, itemSelector) {
    if (getComputedStyle(contElem)["touch-action"] === "none") {
      return;
    } // Already enabled
    let timerId;
    contElem.addEventListener("pointerdown", (event) => {
      if (event.pointerType !== "touch" || isClickInside(event, itemSelector)) {
        return;
      }
      event.preventDefault();
  ...
  function cellPointIntoElem(contElem, x2, y2, rtlWidthOffset) {
    ...
    if (rtlWidthOffset === undefined) {
      ...
      rtlWidthOffset = iSRightToLeft
        ? contElem.scrollWidth - contElem.clientWidth
        : 0;
    }
  ...
  function cellPointIntoElemViewport(contElem, x2, y2) {
    x2 = cellNum(
      x2,
      contElem.scrollLeft,
      contElem.scrollLeft + contElem.clientWidth
    );
    y2 = cellNum(
      y2,
      contElem.scrollTop,
      contElem.scrollTop + contElem.clientHeight
    );
    return [x2, y2];
  }
  ...
  function scrollElem(contElem, x2, y2) {
    ...
    const xDiffAccelerated = Math.round(
      diff * cellNum(Math.abs((x2 - x) / 50), 1, 5)
    );
    const yDiffAccelerated = Math.round(
      diff * cellNum(Math.abs((y2 - y) / 50), 1, 5)
    );
  ...
  async function getAvgFrameTime(frames = 10) {
    return new Promise((resolve) => {
      ...
      requestAnimationFrame(function loop() {
        ...
        resolve(
          Math.round(
            frameTimes.slice(2).reduce((pre, acc) => pre + acc, 0) / frames
          )
        );
      });
    });
